...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        [colors addObject:[UIColor colorWithHexString:@"0x1EBBFE"]];
    }
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x1EBBFE"].CGColor);
    for (int i =0; i<[weekdays count]; i++) {
        NSString *weekdayValue = (NSString *)[weekdays objectAtIndex:i];
        UIFont *font = [UIFont fontWithName:@"Oswald" size:12];
...
-(id)init {
    ...
    if (self) {
        ...
        labelCurrentMonth.font = [UIFont fontWithName:@"Oswald" size:17];
