...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        if ([[UIScreen mainScreen] bounds].size.height == 480) {
            self = [super initWithNibName:@"EditAppointmentViewController4s" bundle:nil];
        } else if ([[UIScreen mainScreen] bounds].size.height == 568) {
            self = [super initWithNibName:@"EditAppointmentViewController" bundle:nil];
        } else if ([[UIScreen mainScreen] bounds].size.height == 667) {
            self = [super initWithNibName:@"EditAppointmentViewController6" bundle:nil];
        } else if ([[UIScreen mainScreen] bounds].size.height == 736) {
            self = [super initWithNibName:@"EditAppointmentViewController6s" bundle:nil];
        }
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    _AddApponimentTable = (UITableView *)[self.view viewWithTag:446];
    ...
    _InvitedUserName = [_EditableObjectCarrier AppointmentWithname];
    _InvitedUserId   = [_EditableObjectCarrier AppointmentWith];
}
...
-(IBAction)DoneButtonClicked:(id)sender
{
    ...
    if ([self CleanTextField:[_TitleTextField text]].length == 0) {
        ...
        NSString *url=[NSString stringWithFormat:@"http://myphpdevelopers.com/dev/ocrscanner/webservice.php?rquest=EditAppointment&AppointmentTitle=%@&AppointmentStartDate=%@&AppointmentEndDate=%@&AppointmentStartTime=%@&AppointmentEndTime=%@&AppointmentWith=%@&AppointmentNote=%@&AppointmentDurationStatus=%@&AppointmentDuration=%@&AppointmentId=%@",[[self CleanTextField:[_TitleTextField text]] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],AppointmentStartDate,AppointmentEndDate,AppointmentStartTime,AppointmentEndTime,_InvitedUserId,[[self CleanTextField:[_DescriptionTextView text]] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],@"",[NSString stringWithFormat:@"%f",timeDifference],[_EditableObjectCarrier AppointmentID]];
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            dispatch_async(dispatch_get_main_queue(), ^(void){
                ...
                if ([[results objectForKey:@"status"] isEqualToString:@"success"]) {
                    UIAlertView *Alertview = [[UIAlertView alloc] initWithTitle:@"Success" message:@"Appointment Edited Successfully" delegate:self cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([indexPath section] == 0)
    {
        ...
        [_TitleTextField setText:[_EditableObjectCarrier AppointmentTitle]];
    ...
    else if ([indexPath section] == 1)
    {
        ...
        else if([indexPath row] == 1)
        {
            ...
            //[_StartDateLabel setText:[NSString stringWithFormat:@"%@ at %@",[_EditableObjectCarrier ApponitmentStartDate],[_EditableObjectCarrier AppointmentStartTime]]];
            [Cell.contentView addSubview:_StartDateLabel];
            
            NSArray *A1 = [[_EditableObjectCarrier ApponitmentStartDate] componentsSeparatedByString:@"-"];
            
            NSString *SM = [self LongDateFromMonth:[A1 objectAtIndex:1]];
            
            NSString *D1 = [NSString stringWithFormat:@"%@-%@-%@",[A1 objectAtIndex:2],SM,[A1 objectAtIndex:0]];
            
            NSArray *A2 = [[_EditableObjectCarrier AppointmentStartTime] componentsSeparatedByString:@":"];
            
            NSString *AMPM = nil;
            NSString *AMPMVal = nil;
            
            if ([[A2 objectAtIndex:0] intValue] > 12) {
                AMPM = @"PM";
                AMPMVal = [NSString stringWithFormat:@"%d",([[A2 objectAtIndex:0] intValue] - 12)];
            } else {
                AMPM = @"AM";
                AMPMVal = [A2 objectAtIndex:0];
            }
            
            NSString *T2 = [NSString stringWithFormat:@"%@:%@ %@",AMPMVal,[A2 objectAtIndex:1],AMPM];
            
            [_StartDateLabel setText:[NSString stringWithFormat:@"%@ %@",D1,T2]];
            
            NSString *dateString = [NSString stringWithFormat:@"%@ %@",D1,T2];
            NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
            [dateFormatter setDateFormat:@"dd-MMM-yyy hh:mm aa"];
            NSDate *dateFromString = [[NSDate alloc] init];
            dateFromString = [dateFormatter dateFromString:dateString];
            _StartDate = dateFromString;
        ...
        else if([indexPath row] == 2)
        {
            ...
            NSArray *A1 = [[_EditableObjectCarrier ApponitmentEndDate] componentsSeparatedByString:@"-"];
            
            NSString *SM = [self LongDateFromMonth:[A1 objectAtIndex:1]];
            
            NSString *D1 = [NSString stringWithFormat:@"%@-%@-%@",[A1 objectAtIndex:2],SM,[A1 objectAtIndex:0]];
            
            NSArray *A2 = [[_EditableObjectCarrier AppointmentEndtime] componentsSeparatedByString:@":"];
            ...
            [_EndDateLabel setText:[NSString stringWithFormat:@"%@ %@",D1,T2]];
    ...
    else if ([indexPath section] == 2)
    {
        ...
        [_InviteLabel setText:[_EditableObjectCarrier AppointmentWithname]];
        [Cell.contentView addSubview:_InviteLabel];
    }
    else if ([indexPath section] == 3)
    {
        [_DescriptionTextView setFrame:CGRectMake(15, 5, 300, 190)];
        [_DescriptionTextView setText:[_EditableObjectCarrier Note]];
        [Cell.contentView addSubview:_DescriptionTextView];
    }
    return Cell;
}
-(NSString* )LongDateFromMonth:(NSString *)PDate
{
    NSString *AString = nil;
    
    if ([PDate isEqualToString:@"01"]) {
        AString     = @"Jan";
    } else if ([PDate isEqualToString:@"02"]) {
        AString     = @"Feb";
    }else if ([PDate isEqualToString:@"03"]) {
        AString     = @"Mar";
    }else if ([PDate isEqualToString:@"04"]) {
        AString     = @"Apr";
    }else if ([PDate isEqualToString:@"05"]) {
        AString     = @"May";
    }else if ([PDate isEqualToString:@"06"]) {
        AString     = @"Jun";
    }else if ([PDate isEqualToString:@"07"]) {
        AString     = @"Jul";
    }else if ([PDate isEqualToString:@"08"]) {
        AString     = @"Aug";
    }else if ([PDate isEqualToString:@"09"]) {
        AString     = @"Sep";
    }else if ([PDate isEqualToString:@"10"]) {
        AString     = @"Oct";
    }else if ([PDate isEqualToString:@"11"]) {
        AString     = @"Nov";
    }else if ([PDate isEqualToString:@"12"]) {
        AString     = @"Dec";
    }
    return AString;
}
